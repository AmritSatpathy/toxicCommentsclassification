...
def get_model():
    ...
    embed_layer = Embedding(max_features,embed_size,input_length=maxlen,weights=[embedding_matrix],trainable=False)(input1)
...
hist = model.fit(X_tra, y_tra, batch_size=batch_size, epochs=3, validation_data=(X_val, y_val),callbacks=[RocAuc], verbose=2)

y_pred = model.predict(x_test, batch_size=1024, verbose=1)

submission = pd.read_csv('../sample_submission.csv')
submission[["toxic", "severe_toxic", "obscene", "threat", "insult", "identity_hate"]] = y_pred
submission.to_csv('subm_capsulenet.csv', index=False)
